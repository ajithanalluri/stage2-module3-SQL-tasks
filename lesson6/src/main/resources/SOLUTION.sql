...
SELECT s.id, s.name, s.birthday, s.groupnumber FROM Student s LEFT JOIN Payment p ON s.id=p.student_id WHERE p.type_id = (SELECT distinct(pt.id) FROM PaymentType pt
LEFT JOIN Payment p ON pt .id=p.type_id WHERE pt.name='WEEKLY');

SELECT s.id, s.name, s.birthday, s.groupnumber FROM Student s LEFT JOIN Mark m ON s.id=m.student_id WHERE m.subject_id= (
